...
const deleteNote = async (entry) => {
  ...
  const deleteAllNotes = async (notes) => {
    ...
    try {
     ...
     if (!response.ok) {
        console.log("Error deleting notes:", response.status);
        alert("Error deleting notes. Please try again.");
      }
    } catch (error) {
        console.log("Fetch function failed:", error)
      } finally {
        setNotes(deleteAllNotesState(notes))
      }
    }
  ...
  const deleteAllNotesState = () => {
      return [] // we can remove all the elements of the array, or we can just return an empty array.
  }
